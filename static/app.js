...
class SuperBizAgentApp {
    ...
    updateUI() {
        ...
        if (this.messageInput) {
            ...
            if (this.currentMode === 'multi_aiops') {
                this.messageInput.placeholder = '请输入要诊断的服务，例如：请诊断 web-service 最近是否存在异常';
            } else {
    ...
    async sendMessage() {
        ...
        try {
            ...
            } else if (this.currentMode === 'multi_aiops') {
                await this.sendMultiAIOpsMessage(message);
            }
        } catch (error) {
    ...
    // 发送多 Agent AIOps 诊断消息
    async sendMultiAIOpsMessage(message) {
        const loadingMessage = this.addLoadingMessage('多 Agent 诊断中...');

        try {
            const targetService = this.inferTargetService(message);

            const response = await fetch(`${this.apiBaseUrl}/aiops_multi`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            const data = await response.json();
            console.log('[sendMultiAIOpsMessage] 响应数据:', data);
            ...
            if (data.code === 200 || data.message === 'success') {
                ...
                const finalReport = result.final_report || '多 Agent 诊断完成，但未返回诊断报告。';
                this.addMessage('assistant', finalReport);
            } else {
                throw new Error(data.message || '多 Agent 诊断失败');
            }
        } catch (error) {
            if (loadingMessage && loadingMessage.parentNode) {
                loadingMessage.parentNode.removeChild(loadingMessage);
    ...
    // 根据用户输入简单推断目标服务
    inferTargetService(message) {
        const text = (message || '').toLowerCase();

        if (
            text.includes('web-service') ||
            text.includes('web服务') ||
            text.includes('web 服务') ||
            text.includes('网页') ||
            text.includes('网站')
        ) {
            return 'web-service';
